...
/** \brief List of regions for device (permanent) storage.
 */
TYPED_ENUM( uint8, atnetRegion )
// clang-format off
{
    FactoryFirmwareRegion = 0u,
    ApplicationFirmwareRegion = 1u,
    CalibrationRegion = 2u,
    IPv4AddressAndPortRegion = 3u,
    EventDataBank1Region = 4u,
    EventDataBank2Region = 5u,
    FactoryReservedRegion = 6u,
    UserDataRegion = 7u,
    DDR2Region = 0xffu
};
// clang-format on
 ...
 * \retval atnetError::ErrNoRunCommand if no commands are provided,
 * \retval atnetError::ErrCannotProcessCommand if the provided command is ill-formed (mismatching number of
 * single or double quotes for instance), if the provided arguments could not be used to feed the command to
 * execute, if the UDP packet could not be written, or if the UDP packet could not be sent, or if the
 * answering packet is not gotten or if the answer indicates a failure,
