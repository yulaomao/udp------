...
#error "Not implemented"
    ...
    Status TrackingSystemAbstract::registerRigidBody( uint64_t serialNbr, const string& geomFileName )
    {
        unique_lock< mutex > lock1( _LibraryProtector, defer_lock );
        unique_lock< mutex > lock2( _enumeratedDevicesProtector(), defer_lock );
        lock( lock1, lock2 );
        ...
        if ( ret == Status::Ok )
        {
            ftkError err( ftkRegisterRigidBody(
              _Library, dev._SerialNumber, geomFileName.length(), geomFileName.c_str() ) );
            if ( err > ftkError::FTK_OK )
            {
                return Status::SdkError;
            }
    ...
    Status TrackingSystemAbstract::registerRigidBody( const string& geomFileName )
    {
        unique_lock< mutex > lock1( _LibraryProtector, defer_lock );
        unique_lock< mutex > lock2( _enumeratedDevicesProtector(), defer_lock );
        lock( lock1, lock2 );
        ...
        for ( size_t idx( 0u ); idx < _numberOfEnumeratedDevices(); ++idx )
        {
            err = ftkRegisterRigidBody(
              _Library, _deviceSerialNumber( idx ), geomFileName.length(), geomFileName.c_str() );
            if ( err > ftkError::FTK_OK )
            {
                retCode = Status::SdkError;
                break;
